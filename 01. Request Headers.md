...
6 | Routing | ID  |  Token ID 0 | Allways zero for WEST
7 | Routing | ID  |  Cloud/[Coin ID](#coin-id) 1 | Always 8 for WEST
...
12 | Presentation | TR | Translation | Future Use. 0 means none. Future use: Can be used to encode text-based protocols. 
13 | Presentation | AI  |AI Translation | Future Use. O means none. What type of AI should be used to translate data that is contained
...
18 | Encryption | SN | Encryption token SN 0| Serial Number of the coin used to encrypt the body. HOB
...
24 | Nonce | NO |  Nonce 0 | The nonce bytes can be used for two purposes in the future if more bytes are needed and the other purpose is random enough
25 | Nonce | NO |  Nonce 1 |
26 | Nonce | NO |  Nonce 2|
27 | Nonce | NO |  Nonce 3|
28 | Nonce | NO |  Nonce 4 |
29 | Nonce | NO |  Nonce 5 
30 | Nonce | NO |  Nonce 6 / Echo 0 | Serves two purposes. These last two Nonce bytes are always echoed back to the client. 
31 | Nonce | NO |  Nonce 7 / Echo 1 | Serves two purposes. This allows the clients to pair requests sent with responses received.
...
3 | Stable Coin | Services that add or subtrack tokens from the WEST RAIDA.
8 | Locker Services | Services that place and retrieve tokens from "RAIDA Lockers."
9 | Change | Breaking larger denominations into smaller ones to make change. Or joining smaller denominations to create larger ones.
...
7 | Blockchain | Services that add or subtrack coins based deposit and withdraw in blockchain accounts. (Not Implemented in V.1)
...
17 | Switch Shard | Moves coins from one shard to another. (Future Use)
...
--- | --- | --- 
 ...
 30 | P'Own with Check Sum and Add | Checks all the ANs added together and then adds one number to these if they are good.
 70 | Identify| Searches all ANs for a match. (Not Implemented)
 
 ## Healing Commands
Code (Decimal) | Command | Description
--- | --- | --- 
 40 |Get Ticket | Returns proof that the coins are good. 
 44 |Get Encrypted Ticket | Returns proof that the coins are good with a ticket that can only be unencrypted by another RAIDA. 
 45 | Fix Encryption | Accepts tickets from many RAIDA to created a shared secret enctyption key from many "key parts". 
 40 |Get Ticket | Returns proof that the coins are good. 
...
Code (Decimal) | Command | Description
--- | --- | --- 
 81 | Put in Locker | Puts coin in a locker that can be opened by a key.
 82 | Get From Locker | Removes coins from locker and destroys locker.
 83 |  Read Meta | Just returns information about what is in the locker such as how many coins are there. 

## Stable Coin Commands
Code (Decimal) | Command | Description
--- | --- | --- 
 100 | Show Stats| Shows how many p'owns there were, how many fixes, how many finds, etc. 
 120 |  Get Available SNS| Shows available serial number. 
 130 |Create Coins| Adds more coins to the total coins on the RAIDA
 140 | Delete Coins| Removes coins from the RAIDA
 150 |Audit Coins | Shows how many coins there are on the RAIDA. 
